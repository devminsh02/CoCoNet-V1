...
추상 기호 `\phi`, `\psi`, `h`, `g` 같은 묶음 함수 대신 **실제 모듈 이름, 레이어 순서, 출력 이름**을 직접 적는 방식으로 전체 구조를 설명한다.
...
2. Branch A의 `uncertainty`는 독립 head가 아니라 **fine logits의 sigmoid entropy**에서 파생된 map이라는 점을 명시한다.
3. Branch A signal은 **raw signal**과 **used signal**을 구분한다.
4. Branch B에서 사용하는 `f_0`, `r`, `f_B`, `c`를 **정의한 뒤 사용**한다.
...
현재 코드에서 graph는 **k-nearest neighbor graph**이며, 기본 `k = 8`이다.
...
즉 message passing은 **feature `T_i`와 좌표 `q_i`를 함께 사용**한다.
...
적어도 **어떤 edge `(i,j)`의 validity를 나타내는 mask**인지 명시해야 한다.
...
중요: `\gamma`는 1채널이 아니라 **256채널 spatial gate**다.
