...
아래 내용은 **현재 업로드된 COCO_V1 코드 기준**으로, `Branch A`와 `Branch B`가 **어떤 입력을 받고**, **어떤 중간 표현을 만들고**, **어떤 출력으로 이어지는지**를 수식과 함께 최대한 자세하게 정리한 문서다.

---

# 0. 큰 그림

이 모델 전체는 대략 다음 흐름으로 볼 수 있다.
...
(z_{\text{coarse}},z_{\text{fine}},o,u,b,f_A)
\;\xrightarrow{\text{Branch B}}\;
(r,z_{\text{cand}},z_{\text{cls}},z_{\text{bdry}},f_B)
...
여기서 각 기호는 다음 의미를 가진다.

- \(x\): 입력 이미지
- \(p_2,p_3,p_4,p_5\): FPN multi-scale feature
- \(z_{\text{coarse}}\): Branch A coarse segmentation logits
- \(z_{\text{fine}}\): Branch A fine segmentation logits
- \(o\): objectness map
- \(u\): uncertainty map
- \(b\): boundary prior
- \(f_A\): Branch A feature
- \(r\): ROI mask
- \(z_{\text{cand}}\): boundary candidate logits
- \(z_{\text{cls}}\): closure logits / closure map
- \(z_{\text{bdry}}\): Branch B refined boundary logits
- \(f_B\): Branch B feature
- \(f_{\text{fused}}\): fusion 결과 feature
- \(z_{\text{final}}\): 최종 camouflage segmentation logits
...
- \(G=16\): Branch B tokenizer의 기본 grid 크기
- 따라서 token 수는 \(N=G^2=256\)

또한 실제 코드의 `ConvBNReLU`, 작은 CNN block, MLP 등은 설명 편의를 위해 \(\phi, \psi, h, g\) 같은 함수 기호로 묶어 적는다.
...
\[
(c_2,c_3,c_4,c_5)=\text{Backbone}(x)
\]

대략적인 해상도와 채널 수는 다음처럼 이해하면 된다.
...
그 다음 simple FPN neck이 이를 모두 256채널로 맞춰서 `p2~p5`를 만든다.
...
- \(W_i\): lateral \(1\times1\) projection
- \(S_i\): smoothing conv block

이다.

결국 FPN 출력은 대략 다음 shape을 가진다.
...
이 브랜치의 목적은 먼저 다음을 만들기 위함이다.

1. **대략 어디에 물체가 있는지**
2. **보다 촘촘한 segmentation 힌트가 무엇인지**
3. **어디가 애매한지**
4. **어디가 경계일 법한지**

즉 Branch A는 최종 contour를 직접 끝까지 닫는 브랜치가 아니라,  
먼저 **semantic prior / objectness prior / uncertainty / boundary prior**를 만들고, 이것을 Branch B와 Fusion에 공급한다.
...
그다음 concat 후 conv block을 태워 fused feature를 만든다.
...
Coarse prediction은 \(f_A\)가 아니라 주로 \(p_4\) 기반의 고수준 feature를 활용한다.

먼저 coarse용 feature를 만든다.

\[
f_{\text{coarse}} = \psi(p_4)
\]

그다음 coarse logit을 예측한다.

\[
z_{\text{coarse}} = U(h_{\text{coarse}}(f_{\text{coarse}}) \to p_2)
\]
...
\[
z_{\text{coarse}} \in \mathbb{R}^{B\times 1 \times H/4 \times W/4}
\]
...
\[
z_{\text{fine}} = h_{\text{fine}}(f_A)
\]

shape는 대략:

\[
z_{\text{fine}} \in \mathbb{R}^{B\times 1 \times H/4 \times W/4}
\]

이건 Branch A의 보다 세밀한 segmentation logits이다.
...
\[
z_{\text{obj}} = h_{\text{obj}}(f_A)
\]

sigmoid를 취하면 objectness map:
...
코드상 실질적으로는 **foreground-like confidence map**에 가깝다.
...
이 map은 “경계처럼 보일 가능성”에 대한 prior다.
...
\[
\bar p = \text{clip}(p,\epsilon,1-\epsilon)
\]
...
uncertainty를 이렇게 이해하면 정확하다.

\[
u = \mathcal{H}(\sigma(z_{\text{fine}}))
\]

즉 uncertainty는 **fine segmentation 결과의 불확실성**이지, 별도의 supervised uncertainty head가 아니다.
...
- \(z_{\text{coarse}}\): 거친 localization
- \(z_{\text{fine}}\): 보다 세밀한 segmentation logits
...
# 4. Branch A의 supervised signal과 loss
...
## 4.1 Final segmentation-like loss 기본식
...
\[
\mathcal L_{\text{seg}}(z,y)
=
\text{BCEWithLogits}(z,y)
+
\lambda_{\text{iou}}
\left(
1-
...
\mathcal L_A
=
\lambda_c \mathcal L_{\text{seg}}(z_{\text{coarse}}, y)
+
\lambda_f \mathcal L_{\text{seg}}(z_{\text{fine}}, y)
+
\lambda_o \mathcal L_{\text{seg}}(z_{\text{obj}}, y)
+
\lambda_b \mathcal L_{\text{seg}}(z_{\text{bp}}, y_{\text{bdry}})
\]
...
즉 uncertainty map에는 **직접적인 supervision이 없다.**
...
y_{\text{bdry}}
=
\mathbf 1\big(\text{dilate}(y)-\text{erode}(y) > 0\big)
\]
...
그다음 concat 후 conv로 base feature를 만든다.
...
\[
r(x,y) \approx P(\text{이 위치는 contour reasoning에 중요하다}\mid o^\*,u^\*,b^\*)
\]
...
\[
z_{\text{cand}}, c \in \mathbb{R}^{B\times 1 \times H/4 \times W/4}
\]

이 단계는 “어느 픽셀이 경계일 가능성이 높은가”를 픽셀 수준에서 먼저 뽑는 단계다.
...
feature를 \(16\times16\) token grid로 줄인다.
...
이 graph는 **feature similarity 기반 graph가 아니다.**  
...
즉 source token feature, destination token feature, 상대 좌표를 함께 본다.

그다음 destination 쪽으로 들어오는 message를 합친다.
...
\[
T^{(\ell+1)} = \text{GraphLayer}(T^{(\ell)}, q, E)
\]
...
z_{\text{cls}}^{\text{tok}}
=
\text{MLP}_{\text{cls}}(T^{(L)})
\in \mathbb{R}^{B\times N\times 1}
\]

이를 다시 \(16\times16\) map으로 reshape한다.

\[
z_{\text{cls}}^{\text{map}}
=
\text{Reshape}(z_{\text{cls}}^{\text{tok}})
...
\[
\tilde z_{\text{cls}} = U(z_{\text{cls}}^{\text{map}} \to f_B)
\]
...
z_{\text{bdry}}
=
h_{\text{ref}}([f_B,\tilde z_{\text{cls}}])
\]
...
\[
z_{\text{bdry}} \in \mathbb{R}^{B\times 1\times H/4 \times W/4}
\]
...
z_{\text{aff}}^{(i,j)}
=
\text{MLP}_{\text{aff}}([T_i,T_j,q_j-q_i])
\]

하지만 현재 공개 v1 코드에서 이 affinity 축은:
...
\[
\text{Branch B}(p_2,p_4,o^\*,u^\*,b^\*)
\mapsto
(r,z_{\text{cand}},z_{\text{cls}},z_{\text{bdry}},f_B)
\]

즉 Branch B를 한 문장으로 요약하면:
...
\phi_F(
[
f_A,\hat f_B,U(z_{\text{bdry}}),U(z_{\text{cls}}),U(u^\*)
]
)
\Big)
\]
...
\[
f_{\text{fused}} = f_A + \gamma \odot \hat f_B
\]
...
\[
z_{\text{final}} = D(f_{\text{fused}})
\]

최종 segmentation probability는

\[
\hat y = \sigma(z_{\text{final}})
\]
...
\[
z_{\text{final}} \in \mathbb{R}^{B\times 1\times H/4 \times W/4}
\]

마지막에는 입력 해상도로 업샘플되어 평가/저장에 쓰인다.
...
\mathcal L_{\text{final}}
=
\mathcal L_{\text{seg}}(z_{\text{final}}, y)
\]
...
\lambda_{\text{bdry}}
\cdot
\text{BCEWithLogits}(z_{\text{bdry}}, y_{\text{bdry}})
\]
...
\lambda_{\text{cand}}
\cdot
\text{BCEWithLogits}(z_{\text{cand}}, y_{\text{bdry}})
\]
...
\frac{1}{2}
\left(
\|z_{\text{bdry}}[:, :, 1:, :] - z_{\text{bdry}}[:, :, :-1, :]\|_1
+
\|z_{\text{bdry}}[:, :, :, 1:] - z_{\text{bdry}}[:, :, :, :-1]\|_1
\right)
\]
...
- 기본 실험의 핵심 기여는 boundary/closure 쪽에 있다
...
\[
z_{\text{coarse}} = U(h_{\text{coarse}}(\psi(p_4)))
\]
...
\[
o = \sigma(h_{\text{obj}}(f_A))
\]

\[
b = \sigma(h_{\text{bp}}(f_A))
\]

\[
u = -(\bar p\log \bar p + (1-\bar p)\log(1-\bar p)),\qquad
\bar p=\text{clip}(\sigma(z_{\text{fine}}))
\]
...
\[
z_{\text{cand}} = h_{\text{cand}}(f_B),\qquad
c=\sigma(z_{\text{cand}})
\]
...
m_{i\to j}
=
\text{MLP}_{\text{msg}}([T_i,T_j,q_j-q_i])
\]
...
\[
T'_j=\text{LN}(T_j+\text{MLP}_{\text{upd}}([T_j,a_j]))
\]
...
\[
z_{\text{cls}}^{\text{tok}}=\text{MLP}_{\text{cls}}(T^{(L)})
\]

\[
z_{\text{cls}}^{\text{map}}=\text{Reshape}(z_{\text{cls}}^{\text{tok}})
\]
...
\[
z_{\text{bdry}}=h_{\text{ref}}([f_B,\tilde z_{\text{cls}}])
\]
...
\phi_F(
[
f_A,\hat f_B,U(z_{\text{bdry}}),U(\tilde z_{\text{cls}}),U(u^\*)
]
)
\Big)
\]
...
> **Branch A가 전역 semantic prior(objectness, uncertainty, boundary prior, coarse/fine segmentation)를 만들고,  
> Branch B가 그 prior를 바탕으로 ROI를 정하고 boundary fragment를 token graph로 reasoning하여 closure-aware boundary를 만든 뒤,  
> Fusion이 A를 본체로 두고 B를 선택적으로 주입하여 최종 segmentation을 완성한다.**
...
(o^\*,u^\*,b^\*)
\rightarrow
(r,z_{\text{cand}},T,E,z_{\text{cls}},z_{\text{bdry}},f_B)
\rightarrow
f_{\text{fused}}
\rightarrow
z_{\text{final}}
...
필요하면 다음엔 이 내용을 이어서  
`global_objectness_branch.py`, `contour_closure_branch.py`, `cod_model.py`, `loss_manager.py` 기준으로 **코드 한 줄 단위 해설 버전**으로 다시 써주겠다.